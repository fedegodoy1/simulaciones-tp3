...
public class ExponencialTestTable extends javax.swing.JFrame {
    ...
    public ExponencialTestTable(JFrame parent, Controller cont, float[] values, String[] datos, int intervalos) {
        rango = calcularRango(values, intervalos);
        controller = cont;
        float[][] matriz = calculator.matrizFrecuenciaExponencial(values, rango, intervalos, minimo);
...
//      //  agregarHistograma();
    ...
    public double estadisticoPruebaTotal(float[][] matriz, int frecEsp) {
        double res = 0;
        for (int i = 0; i < matriz.length; i++) {
            res += (double) (Math.pow(matriz[i][2] - frecEsp, 2)) / frecEsp;
        }
        return res;
    }
